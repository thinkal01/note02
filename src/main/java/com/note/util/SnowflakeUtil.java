...
/**
 * Twitter_Snowflake
 * snowflake是Twitter开源的分布式ID生成算法，结果是一个long型ID。其核心思想是：
 * 高位随机+毫秒数+机器码(数据中心+机器id)+10位的流水号码
 * SnowFlake的结构如下(每部分用-分开):
 * 0 - 0000000000 0000000000 0000000000 0000000000 0 - 00000 - 00000 - 000000000000
 * 所有位数加起来共64位，恰好是一个Long型(转换为字符串长度为18).单台机器实例，通过时间戳保证前41位是唯一的，
 * 1位标识，由于long基本类型在Java中是带符号的，最高位是符号位，正数是0，负数是1，id一般是正数，最高位是0
 * 41位时间截(毫秒级)，注意，41位时间截不是存储当前时间的时间截，而是存储时间截的差值（当前时间截 - 开始时间截)得到的值），
 * 开始时间截，一般是id生成器开始使用的时间，由程序来指定（如下面程序IdWorker类的startTime属性）。
 * 41位时间截，可以使用69年，年T = (1L << 41) / (1000L * 60 * 60 * 24 * 365) = 69
 * 10位的数据机器位，datacenterId*workerId最多支持部署1024个节点
 * 通过对每个机器实例分配不同的datacenterId和workerId避免中间的10位碰撞。
 * 包括5位datacenterId和5位workerId(最大支持2^5＝32个，二进制表示从00000-11111，也即是十进制0-31)，
...
public class SnowflakeUtil {
    ...
    private void init() throws UnknownHostException {
        ...
        // 通过ip最后一位的高低四位获取id
        datacenterId = address[3] & 0b11110000;
        workerId = address[3] & 0b00001111;
