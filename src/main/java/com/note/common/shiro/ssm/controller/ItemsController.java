...
import com.note.common.shiro.ini.controller.validation.ValidGroup1;
import com.note.common.shiro.ini.po.Items;
import com.note.common.shiro.ini.po.QueryVo;
import com.note.common.shiro.ssm.service.ItemsService;
...
public class ItemsController {
    ...
    public ModelAndView queryItems(HttpServletRequest request) throws Exception {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            items.setDetail("detail" + i);
            items.setName("name" + i);
            items.setId(i);
            items.setPrice((float) i);
            itemsList.add(items);
        }
        // 调用service查询商品列表
    ...
    public String editItemsListSubmit(QueryVo itemsQueryVo) throws Exception {
    ...
    public @ResponseBody
    Items viewItems(@PathVariable("id") Integer id) throws Exception {
        // 调用 service查询商品信息
        Items itemsCustom = itemsService.findItemsById(id);
        return itemsCustom;
    }
    ...
    // request.getRequestDispatcher("/WEB-INF/jsp/editItem.jsp").forward(request,
    // response);
    //
    // }
    ...
    @RequiresPermissions("item:update")
    // 执行此方法需要"item:update"权限
    public String editItemSubmit(Model model, Integer id, @Validated(value = {ValidGroup1.class}) @ModelAttribute(value = "itemsCustom") Items itemsCustom,
