...
/*
    ...
    每个类都会使用自己的类加载器（即加载自身的类加载器）去加载其他类（指的是所依赖的来）
    如果是ClassX引用了ClassY，那么CLassX的类加载器就会去加载ClassY（前提是ClassY尚未被加载）

    线程上下文类加载器（Context Classloader）

    线程上下文类加载器是从JDK1.2开始引入的，类Thread中的getContextClassLoader() 与 setContextClassLoader(ClassLoader cl)
    分别用来获取和设置上下文类加载器。
...
public class Test24 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Thread.class.getClassLoader()); // 位于java.lang包下，由启动类加载器加载
    }
}
