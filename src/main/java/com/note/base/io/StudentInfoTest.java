...
思路：
1，3门课的成绩都是数据，为了便于操作，将其封装到学生对象中。学生本身就是问题领域中涉及的对象，对学生描述。
学生：姓名，语文成绩，英语成绩，数学成绩，总分.
2，数据来源于键盘录入，将这些数据都封装到每一个学生对象中。
3，按照总分排个序，但是这些数据都存储到了学生对象中，其实是学生对象排序。
...
public class StudentInfoTest {
    public static void main(String[] args) throws IOException {
        Comparator<Student> comp = Collections.reverseOrder(new ComparatorByMath());
        Set<Student> set = StudentInfoTool.getStudents(comp);
        File file = new File("stuinfo.txt");
        StudentInfoTool.write2File(set, file);
    }

    static class ComparatorByMath implements Comparator<Student> {
        @Override
